...
const UserProfile = () => {
  const { user: backendUser, logout } = useAuthStore();
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-500 to-green-600 flex items-center justify-center p-4">
      ...
      >
        <h2 className="text-4xl font-extrabold text-center bg-white text-transparent bg-clip-text">
          Welcome, {displayUser.name.split(" ")[0]}!
        </h2>
        ...
        >
          ...
          <p className="text-white/90"><span className="font-medium">Name:</span> {displayUser.name}</p>
          <p className="text-white/90"><span className="font-medium">Email:</span> {displayUser.email}</p>
          <p className="text-white/90"><span className="font-medium">Role:</span> {displayUser.role || "User"}</p>
        </motion.div>
        ...
        >
          ...
          <p className="text-white/90"><span className="font-bold">Joined:</span> {new Date(displayUser.createdAt).toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}</p>
          <p className="text-white/90"><span className="font-bold">Last Login:</span> {formatDate(displayUser.lastLogin)}</p>
          <p className="text-white/90"><span className="font-bold">Posts Created:</span> {displayUser.posts || 0}</p>
        </motion.div>
